...
import { listFieldPortfolios, listFieldProperties, createFieldProperty, createFieldBuilding, createFieldOpening } from "../lib/api";
interface Building { id:string; name:string }
interface Facility { id:string; name:string; buildings:Building[] }
...
export function SetupOpeningPage() {
  ...
  useEffect(()=>{Promise.all([listFieldPortfolios(),listFieldProperties()]).then(([p,f])=>{
  ...
  return <div className="screen"><Link to="/scan">Back to openings</Link><h1>Facilities and openings</h1>
    ...
    <div className="field"><label htmlFor="facility">Facility</label><select id="facility" value={facility} onChange={e=>{setFacility(e.target.value);setBuilding("");}}><option value="">Select facility…</option>{facilities.map(f=><option key={f.id} value={f.id}>{f.name}</option>)}</select></div>
    ...
    {canCreate&&<>
    <form onSubmit={e=>{e.preventDefault();void perform(async()=>{const row=await createFieldProperty({portfolio_id:portfolio,name:facilityName.trim(),property_type:type});setFacilities(prev=>[...prev,{...row,buildings:[]}]);setFacility(row.id);setBuilding("");setFacilityName("");setNotice("Facility saved.");});}}>
