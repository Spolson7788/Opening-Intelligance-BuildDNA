...
function on(id,fn){$(id).onclick=async()=>{const b=$(id);b.disabled=true;try{await fn();}catch(e){msg(e.message);}finally{b.disabled=false;}};}
...
async function renderOpening(){const stamp=++epoch,a=opening;const results=await Promise.all([sb.from('opening_structure').select('*').eq('assembly_id',a.id),sb.from('opening_components').select('*').eq('assembly_id',a.id)]);if(stamp!==epoch)return;structures=must(results[0]);components=must(results[1]);$('structure').replaceChildren();
 ...
 $('components').replaceChildren(...components.map(c=>{const el=document.createElement('article');const text=document.createElement('span');text.textContent=[c.component_class,c.manufacturer,c.model,c.condition,structures.find(s=>s.id===c.structure_id)?.kind||'opening'].filter(Boolean).join(' · ');const b=document.createElement('button');b.textContent='Edit';b.onclick=()=>edit(c);el.append(text,b);return el;}));
 $('photoTarget').replaceChildren(option('opening','Opening'),...structures.map(s=>option('structure:'+s.id,s.kind.replaceAll('_',' '))),...components.map(c=>option('component:'+c.id,componentLabel(c,structures))));
 await showPhotos(a,stamp);edit();$('purchasing').textContent='';
}
function edit(c={}){for(const [id,k] of Object.entries({componentId:'id',componentClass:'component_class',manufacturer:'manufacturer',model:'model',documentUrl:'document_url'}))$(id).value=c[k]||'';$('componentParent').value=c.structure_id||'';$('condition').value=c.condition||'unverified';$('disposition').value=c.disposition||'refused';$('identitySource').value=c.identity_source||'unresolved';$('reviewed').checked=!!c.reviewed;$('product').value=c.product_id||'';}
async function showPhotos(a,stamp){const rows=must(await sb.from('assembly_photos').select('*').eq('assembly_id',a.id).order('taken_at'));const cards=await Promise.all(rows.map(async r=>{const card=document.createElement('article');const caption=document.createElement('p');caption.textContent=photoScopeLabel(r,[...structures,...components]);card.append(caption);const signed=await sb.storage.from('opening-photos').createSignedUrl(r.storage_path,120);if(signed.error){card.append(document.createTextNode('Photograph unavailable — retry when authorized and online.'));}else{const img=document.createElement('img');img.alt=caption.textContent;img.src=signed.data.signedUrl;card.append(img);}return card;}));if(stamp===epoch)$('photos').replaceChildren(...cards);}
on('signin',async()=>{must(await sb.auth.signInWithPassword({email:$('email').value.trim(),password:$('password').value}));$('password').value='';await boot();});
...
on('savePhoto',async()=>{const file=$('photoFile').files[0];if(!file)throw Error('Choose a photograph');const id=crypto.randomUUID(),target=$('photoTarget').value.split(':');const payload={id,assembly_id:opening.id,facility_id:opening.facility_id,structure_id:target[0]==='structure'?target[1]:null,component_id:target[0]==='component'?target[1]:null,storage_path:opening.facility_id+'/'+opening.id+'/'+id};await enqueue(user.id,url,'photo',payload,file);$('photoFile').value='';await sync();await renderOpening();});
on('retry',async()=>{await sync();if(opening)await renderOpening();});
